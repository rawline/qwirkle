...
export async function auth(login, password) {
  ...
  return request('/auth.php', { method: 'POST', body: { login, password }, token: null })
}

export async function register(login, password) {
  // returns { token } or { error }
  return request('/register.php', { method: 'POST', body: { login, password }, token: null })
}
...
export async function createGame({ seats = 4, move_time = 60 } = {}) {
  ...
  return request('/create_game.php', { method: 'POST', body: { seats, move_time } })
}

export async function getGameState({ token, playerId }) {
  const t = token || getToken()
  const params = new URLSearchParams({ p_token: String(t), p_player_id: String(playerId) })
...
export async function placeTile({ game_id, player_id, tile_id, x, y }) {
  return request('/place_tile.php', { method: 'POST', body: { game_id, player_id, tile_id, x, y } })
}

export async function finishTurn({ game_id, player_id }) {
  return request('/finish_turn.php', { method: 'POST', body: { game_id, player_id } })
}
